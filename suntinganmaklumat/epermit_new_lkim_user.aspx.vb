...
Partial Class UserManagement_LKIM_new_lkim_user
    ...
    Protected Sub NewUser_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NewUser.Click
        ...
        sqlText = "select count(*) from EPERMIT_MAINTENANCE where epermit_serial_no ='" & textbox1.Text & "'"
        Dim rcount As Integer = opClass.SingleFieldResults(sqlText)

        If rcount > 0 Then
            Dim popupScript As String = "<script language='javascript'>" & _
                                         "alert('Kod e-permit Sudah Wujud!!!\nPilih kod Yang Lain.');" & _
                                         "</script>"
            Response.Write(popupScript)
        Else
            sqlText = "INSERT INTO [W-ICCS].[dbo].[EPERMIT_MAINTENANCE] (epermit_serial_no , epermit_code , epermit_desc , epermit_cost , status, edate ,  userid)" & _
            "VALUES('" & textbox1.Text & "','" & textbox2.Text & "','" & TextBox3.Text & "','" & TextBox4.Text & "','" & DropDownList1.Text & "','" & TextBox5.Text & "','" & TextBox6.Text & "')"
