...
Partial Class UserManagement_LKIM_edit_lkim_user
    ...
    Protected Sub NewUser_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NewUser.Click
        ...
        Dim sqlText As String = "select count(*) from pengimport where kod_pengimport='" & textbox1.Text & "'"
        Dim rcount As Integer = opClass.SingleFieldResults(sqlText)

        sqlText = "UPDATE pengimport set kod_pengimport = '" & textbox1.Text & "', nama_syarikat_import = '" & textbox2.Text & "', alamat_syarikat_import = '" & TextBox3.Text & "', no_lesen = '" & TextBox4.Text & "', alamat_menyurat = '" & TextBox5.Text & "', nama_pemilik = '" & TextBox6.Text & "', no_kp = '" & TextBox7.Text & "', no_tel = '" & TextBox8.Text & "' where kod_pengimport ='" & textbox1.Text & "'"
        ...
        If opClass.ErrorSignal = False Then
            Dim popupScript As String = "<script language='javascript'>" & _
                                    "alert('Maklumat Telah Selamat Dikemaskini!!!');location.href='pengimporttm.aspx';" & _
                                    "</script>"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            sqlText = "SELECT Kod_Pengimport , Nama_Syarikat_Import , Alamat_Syarikat_Import, No_Lesen , Alamat_Menyurat , Nama_Pemilik , No_KP , No_Tel  from pengimport WHERE kod_pengimport='" & Request("user") & "'"
            ...
            If sqlRdr.HasRows Then
                ...
                TextBox6.Text = sqlRdr.GetValue(5).Trim & ("")
                TextBox7.Text = sqlRdr.GetValue(6).Trim & ("")
                TextBox8.Text = sqlRdr.GetValue(7).Trim & ("")
                sqlRdr.Close()
                sqlRdr = Nothing
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            sqlText = "update pengimport set status='no' where kod_pengimport='" & textbox1.Text & "'"
            'sqlText = "delete from ikan_caj where id='" & textbox1.Text & "'"
            opClass.InsertData(sqlText)

            If opClass.ErrorSignal = False Then
                Dim popupScript As String = "<script language='javascript'>" & _
                                        "alert('Maklumat Telah Selamat Dihapuskan!!!');location.href='pengimporttm.aspx';" & _
                                        "</script>"
                Response.Write(popupScript)
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
