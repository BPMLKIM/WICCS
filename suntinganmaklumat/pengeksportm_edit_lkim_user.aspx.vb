...
Partial Class UserManagement_LKIM_edit_lkim_user
    ...
    Protected Sub NewUser_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NewUser.Click
        ...
        Dim sqlText As String = "select count(*) from pengeksport where kod_pengeksport='" & textbox1.Text & "'"
        Dim rcount As Integer = opClass.SingleFieldResults(sqlText)

        sqlText = "UPDATE pengeksport set kod_pengeksport = '" & textbox1.Text & "', nama_syarikat_eksport = '" & textbox2.Text & "', alamat_syarikat_eksport = '" & TextBox3.Text & "', no_lesen = '" & TextBox4.Text & "', tarikh_dimasukkan = '" & TextBox5.Text & "' where kod_pengeksport ='" & textbox1.Text & "'"
        ...
        If opClass.ErrorSignal = False Then
            Dim popupScript As String = "<script language='javascript'>" & _
                                    "alert('Maklumat Telah Selamat Dikemaskini!!!');location.href='pengeksportm.aspx';" & _
                                    "</script>"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            sqlText = "SELECT kod_pengeksport , nama_syarikat_eksport, alamat_syarikat_eksport, no_lesen, tarikh_dimasukkan from pengeksport WHERE kod_pengeksport='" & Request("user") & "'"
            ...
            If sqlRdr.HasRows Then
                ...
                TextBox5.Text = sqlRdr.GetValue(4)
                sqlRdr.Close()
                sqlRdr = Nothing
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
                                               "alert('pengeksport Tidak Wujud!!!');history.back();" & _
                                               "</script>"
                Response.Write(popupScript)
            End If
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            sqlText = "update pengeksport set status='no' where kod_pengeksport='" & textbox1.Text & "'"
            'sqlText = "delete from ikan_caj where id='" & textbox1.Text & "'"
            opClass.InsertData(sqlText)

            If opClass.ErrorSignal = False Then
                Dim popupScript As String = "<script language='javascript'>" & _
                                        "alert('Maklumat Telah Selamat Dihapuskan!!!');location.href='pengeksportm.aspx';" & _
                                        "</script>"
                Response.Write(popupScript)
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
