...
Partial Class pengeksport_kecuali_new
    ...
    Protected Sub NewUser_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NewUser.Click
        ...
        sqlText = "select count(*) from pengeksport_kecuali where kod_pengeksport='" & searchKodPengeksport.Text & "' and kod_ikan='" & Ikan.Text & "' and t_mula='" & tmula & "' and t_tamat='" & ttamat & "' and status='yes'"
        Dim rcount As Integer = opClass.SingleFieldResults(sqlText)

        If rcount > 0 Then
            Dim popupScript As String = "<script language='javascript'>" & _
                                         "alert('Kod Pengeksport Sudah Wujud!!!\nPilih Kod Pengeksport Yang Lain.');" & _
                                         "</script>"
        ...
        Else
            ...
            sqlText = "INSERT INTO [w-iccs].[dbo].[pengeksport_kecuali] " & _
                      "(kod_pengeksport,kod_ikan, nama_syarikat_eksport,t_mula, t_tamat,status) values('" & searchKodPengeksport.Text & "','" & Ikan.Text & "','" & Pengeksport.Text & "','" & tmula & _
                      "',   '" & ttamat & "','yes')"
