...
Partial Class UserManagement_LKIM_edit_lkim_user
    ...
    Protected Sub NewUser_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NewUser.Click
        ...
        Dim sqlText As String = "select count(*) from negara where kod_negara='" & textbox1.Text & "'"
        Dim rcount As Integer = opClass.SingleFieldResults(sqlText)

        sqlText = "UPDATE negara set kod_negara = '" & textbox1.Text & "', nama_negara = '" & textbox2.Text & "' , kumpulan_negara = '" & list5.Text & "' where kod_negara ='" & textbox1.Text & "'"
        ...
        If opClass.ErrorSignal = False Then
            Dim popupScript As String = "<script language='javascript'>" & _
                                    "alert('Maklumat Telah Selamat Dikemaskini!!!');location.href='negara.aspx';" & _
                                    "</script>"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            sqlText = "SELECT kod_negara ,nama_negara,kumpulan_negara from negara WHERE kod_negara='" & Request("user") & "'"
            ...
            If sqlRdr.HasRows Then
                ...
                list5.Text = sqlRdr.GetValue(2).Trim & ("")
                sqlRdr.Close()
                sqlRdr = Nothing
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
                                               "alert('negara Tidak Wujud!!!');history.back();" & _
                                               "</script>"
                Response.Write(popupScript)
            End If
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            sqlText = "update negara set status='no' where kod_negara='" & textbox1.Text & "'"
            'sqlText = "delete from ikan_caj where id='" & textbox1.Text & "'"
            opClass.InsertData(sqlText)

            If opClass.ErrorSignal = False Then
                Dim popupScript As String = "<script language='javascript'>" & _
                                        "alert('Maklumat Telah Selamat Dihapuskan!!!');location.href='negara.aspx';" & _
                                        "</script>"
                Response.Write(popupScript)
            Else
                Dim popupScript As String = "<script language='javascript'>" & _
