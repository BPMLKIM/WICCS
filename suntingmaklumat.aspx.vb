...
Partial Class suntingmaklumat
    ...
    Public connectionstring As String = "Data Source=localhost;Initial Catalog=W-ICCS;User ID=sa;Password=quest2k"
    ...
    Protected Sub DropDownList2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DropDownList2.SelectedIndexChanged
        ...
        If DropDownList2.SelectedIndex <> 0 Then
            ...
            If Label7.Text = "01" Then
                view_agent(nak)
            ElseIf Label7.Text = "02.1" Then
                view_bentuk_ikan_1(nak)
            ElseIf Label7.Text = "02.2" Then
                view_bentuk_ikan_2(nak)
            ElseIf Label7.Text = "02.3" Then
                view_bentuk_ikan_3(nak)
            ElseIf Label7.Text = "02.4" Then
                view_bentuk_ikan_4(nak)
            ElseIf Label7.Text = "02.5" Then
    ...
    Function show_caj_ikan()
        ...
        Dim myconn As New SqlConnection(connectionstring)
        Dim mycmd As New SqlCommand("SELECT id, kategori_ikan FROM ikan_caj where id <>'00000'ORDER BY kategori_ikan", myconn)
        Dim item2 As ListItem
        Dim SqlRdr As SqlDataReader
        myconn.Open()
        SqlRdr = mycmd.ExecuteReader(System.Data.CommandBehavior.CloseConnection)
        ...
        While SqlRdr.Read
            item2 = New ListItem()
            item2.Text = SqlRdr.Item("kategori_ikan")
    ...
    Function show_jenis_ikan()
        ...
        While SqlRdr.Read
            ...
            DropDownList2.Visible = True
        End While
        DropDownList2.Items.Insert(0, "-- Select Kategori jenis Ikan --")
        Label7.Text = DropDownList1.SelectedItem.Value
        Label8.Text = DropDownList2.SelectedItem.Value
    ...
    Function show_epermit()
        ...
        TextBox3.Enabled = True
        Label1.Visible = True
        Label1.Text = "Kod E-permit :"
        Label2.Visible = True
        Label2.Text = "E-permit description :"
        Label3.Visible = True
        Label3.Text = "E-permit Cost :"
        Label4.Visible = True
        Label4.Text = "E-permit Status :"

        Dim myconn As New SqlConnection(connectionstring)
        Dim mycmd As New SqlCommand("SELECT epermit_code, epermit_desc FROM epermit_maintenance where epermit_code <>'00000'ORDER BY epermit_desc", myconn)
        Dim item2 As ListItem
        Dim SqlRdr As SqlDataReader
        myconn.Open()
        SqlRdr = mycmd.ExecuteReader(System.Data.CommandBehavior.CloseConnection)
        ...
        If SqlRdr.Read Then
            item2 = New ListItem()
            item2.Text = SqlRdr.Item("epermit_desc")
            item2.Value = SqlRdr.Item("epermit_code")
            DropDownList2.Items.Add(item2)
            '   label8.Text = DropDownList2.Items(item2)
            DropDownList2.Visible = True
        Else
    ...
    Function view_epermit(ByVal result As String)
        Dim myconn As New SqlConnection(connectionstring)
        Dim mycmd As New SqlCommand("SELECT epermit_code AS Expr1, epermit_desc AS Expr2, epermit_cost, status AS Expr4 from epermit_maintenance where epermit_code= '" & result & "' ", myconn)
        Dim SqlRdr As SqlDataReader
        myconn.Open()
        SqlRdr = mycmd.ExecuteReader(System.Data.CommandBehavior.CloseConnection)
        If SqlRdr.Read Then
            TextBox1.Text = SqlRdr.Item("Expr1").Trim & ("")
            TextBox2.Text = SqlRdr.Item("Expr2").Trim & ("")
            TextBox3.Text = SqlRdr.Item("Expr2").Trim & ("")
            DropDownList3.Text = SqlRdr.Item("Expr4")
            TextBox1.Enabled = False
            add.Visible = True
            add.Enabled = True
            save.Enabled = True
    ...
    Function save_caj_ikan()
        ...
        str = "UPDATE ikan_caj set id = '" & TextBox1.Text & "', kategori_ikan = '" & TextBox2.Text & "', jenis_urusan = '" & TextBox3.Text & "', kadar_peti_kecil = '" & TextBox4.Text & "', kadar_peti_besar = '" & TextBox5.Text & "', kadar_ekor = '" & TextBox9.Text & "', kadar_kuantiti = '" & TextBox10.Text & "' where id ='" & TextBox1.Text & "'"
    ...
    Function save_epermit()
        ...
        Dim myconn As New SqlConnection(connectionstring)
        Dim mycmd As New SqlCommand("select * from epermit_maintenance where epermit_code ='" & TextBox1.Text & "'", myconn)
        Dim str As String
        myconn.Open()

        str = "UPDATE epermit_maintenance set epermit_code = '" & TextBox1.Text & "', epermit_desc = '" & TextBox2.Text & "', epermit_cost = '" & TextBox3.Text & "', status = '" & DropDownList3.Text & "' where epermit_code ='" & TextBox1.Text & "'"
    ...
    Function save_add_agen_pengangkutan()
        ...
        Else
            ...
            str = "INSERT INTO AGEN_PENGANGKUTAN(Kod_Agen_Pengangkutan, Nama_Agen_Pengangkutan, Alamat_Agen_Pengangkutan, Deposit_Semasa, Transaksi_Semasa)" _
            & "VALUES('" & TextBox6.Text & "','" & TextBox7.Text & "','" & TextBox8.Text & "','0','0')"
    ...
    Function delete_epermit()
        ...
        str = "DELETE FROM epermit_maintenance where epermit_code ='" & TextBox1.Text & "'"
    ...
    Function datagrid_agen()
        Dim myconn As New SqlConnection(connectionstring)
        Dim sql As String = "SELECT KOD_AGEN_PENGANGKUTAN,NAMA_AGEN_PENGANGKUTAN,ALAMAT_AGEN_PENGANGKUTAN from  AGEN_PENGANGKUTAN where kod_agen_pengangkutan <>'00000' ORDER BY NAMA_AGEN_PENGANGKUTAN "
    ...
    Function datagrid_caj_perkhidmatan()
        Dim myconn As New SqlConnection(connectionstring)
        Dim sql As String = "SELECT kod_caj, nama_caj FROM caj_perkhidmatan where kod_caj <>'00000'ORDER BY nama_caj"
        Dim Cmd As New SqlClient.SqlCommand(sql, myconn)
        myconn.Open()
        Dim myAdapter As New SqlClient.SqlDataAdapter(Cmd)
        Dim ds As New DataSet()
    ...
    Function datagrid_caj_ikan()
        Dim myconn As New SqlConnection(connectionstring)
        Dim sql As String = "SELECT id, kategori_ikan FROM ikan_caj where id <>'00000'ORDER BY kategori_ikan"
        Dim Cmd As New SqlClient.SqlCommand(sql, myconn)
        myconn.Open()
        Dim myAdapter As New SqlClient.SqlDataAdapter(Cmd)
        Dim ds As New DataSet()
    ...
    Function datagrid_lesen()
        Dim myconn As New SqlConnection(connectionstring)
        Dim sql As String = "SELECT No_lesen, siri_lesen FROM lesen where no_lesen <>'00000'ORDER BY siri_lesen"
        Dim Cmd As New SqlClient.SqlCommand(sql, myconn)
        myconn.Open()
        Dim myAdapter As New SqlClient.SqlDataAdapter(Cmd)
        Dim ds As New DataSet()
