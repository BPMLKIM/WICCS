...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Request("urusan").ToUpper() = "I" Then
            'import table
            tabletitle = "ITEM_ISYTIHAR_I"
            tabletitle1 = "PENGISYTIHARAN_I"
            tabletitle2 = "pembayaran_caj_ikan"
            textfield = "No_SKPI"
            title = "import"
        ...
        Else
            ...
            tabletitle2 = "pembayaran_caj_ikan_eksport"
            textfield = "No_SKPE"
            title = "eksport"
        ...
        Dim sqlIkan As String = "SELECT sum(TotalCajIkan) as TotalCajIkan, sum(Gst) as Gst , sum(TotalCajIkan+Gst) as TotalCajIkanGst From " & Trim(tabletitle1) & " " _
        ...
        Dim jumlahgst As Decimal
        Dim jumlahcajikangst As Decimal
        ...
        If SQLReader.HasRows Then
            ...
            jumlahgst = CDec(SQLReader("Gst"))
            jumlahcajikangst = CDec(SQLReader("TotalCajIkanGst"))

            '--pelarasan gst
            pelarasangst = jumlahcajikangst
            ...
            If afterDecimalSeparator > 1 Then
                ...
                If pelarasangst = 1 Then
                    ...
                    jumcajslpspelarasangst = jumlahcajikangst - jumpelarasangst
        ...
        viewborangikan = "Caj Perkhidmatan Ikan" & "`" & CDec(jumlahcajikan) & "`" & CDec(jumlahgst) & "`" & CDec(jumlahcajikangst) & "`" & (descpelarasangst) & "`" & CDec(jumcajslpspelarasangst) & "`" & "LKIM" & "`" & "PENGISYTIHARAN" & "~"
        ...
        'edited by kmz for caj eksport
        Dim sqlBorang1 As String = "select no_barcode from " + tabletitle2 + " where No_barcode = '" & Trim(Request("strBarcode")) & "'"
        'end of edited by kmz for caj eksport
    ...
    Protected Sub BAYAR_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdBayar.Click, strbuttonL.Click
        ...
        'edited by kmz for caj eksport
        Dim tableinvois As String = Nothing
        Dim tablecaj As String = Nothing
        Dim tablecaj1 As String = Nothing
        Dim tablecaj2 As String = Nothing

        If Request("urusan") = "I" Then
            tableinvois = "NO_INVOIS"
            tablecaj = "ITEM_CAJ"
            tablecaj1 = "PEMBAYARAN_CAJ_IKAN"
            tablecaj2 = "PEMBAYARAN_CAJ"
        Else
            tableinvois = "NO_INVOIS_EKSPORT"
            tablecaj = "ITEM_CAJ_EKSPORT"
            tablecaj1 = "PEMBAYARAN_CAJ_IKAN_EKSPORT"
            tablecaj2 = "PEMBAYARAN_CAJ_EKSPORT"
        End If
        'end of edited by kmz for caj eksport
            ...
            If (Trim(cara_pembayaran.SelectedValue) = "CEK" Or Trim(cara_pembayaran.SelectedValue) = "CEK/TUNAI") Or Trim(sender.Id) = "strbuttonL" Then
                ...
                If SQLReader.HasRows Then
                    ...
                    If Trim(cara_pembayaran.SelectedValue) = "CEK" Then
                        ...
                        total_caj_kredit = CDec(transaksi_semasa) + CDec(arrBayarCol(5))
                        'end of shah edit for caj gst
                ...
                Else
                    ...
                    If Trim(cara_pembayaran.SelectedValue) = "CEK" Then
                        ...
                        total_caj_kredit = CDec(arrBayarCol(5))
                        'end of shah edit for caj gst
        ...
        If Trim(sender.Id) = "cmdBayar" Then
            ...
            'edited by kmz for caj eksport
            Dim intInvois As String = "select no_invois from " + tableinvois + " where no_barcode='" & Trim(txtRujukan.Text) & "'"
            'end of edited by kmz for caj eksport
            ...
            'Dim totalTransaksiSemasa As Double
            ...
            Call SaveSistemQ()
            'edited by kmz for caj eksport
            Dim intbil As String = "select bil_item from " + tablecaj + " where no_barcode='" & Trim(txtRujukan.Text) & "' order by bil_item desc"
            'end of edited by kmz for caj eksport
            ...
            If Trim(cara_pembayaran.SelectedValue) = "CEK" Or Trim(cara_pembayaran.SelectedValue) = "TUNAI" Then
                ...
                'edited by kmz for caj eksport dan gst
                Dim sqlText1 As String = "INSERT INTO [W-ICCS].[dbo].[" + tablecaj + "]" _
            ...
            ElseIf Trim(cara_pembayaran.SelectedValue) = "CEK/TUNAI" Then
                ...
                Dim sqlText2 As String = "INSERT INTO [W-ICCS].[dbo].[" + tablecaj + "]" _
               & " ([No_Barcode], [Bil_Item], [Kod_Service_Caj], [Nilai_Caj], [Gst],[Nilai_Caj_Gst],[Pelarasan_Gst],[Nilai_Caj_Pelarasan_Gst],[Cara_Pembayaran],[No_Cek],[Dicaj_Oleh],[Tempat_Isytihar],[Nama_Pegawai],[No_Pegawai],site_code)" _
                ...
                'end of edited by kmz for caj eksport dan gst

            End If

            'edited by kmz for caj eksport
            Dim strBayar2 = "insert into " + tablecaj1 + "(no_barcode,no_invois,siri_module,kod_agen,print_status,site_code) " _
            ...
            'edited by kmz for caj eksport dan gst
            Dim rstCaj As String = "Select sum(nilai_caj_pelarasan_gst) as tunaicaj FROM " + tablecaj + " WHERE Cara_Pembayaran='TUNAI' and kod_service_caj='Caj Perkhidmatan Ikan' and No_Barcode = '" & Trim(txtRujukan.Text) & "'"
            ...
            If SQLReader.HasRows Then
                ...
                Else
                    txtTotalCajTunai = Math.Round(CDec(SQLReader("tunaicaj")), 2)
            ...
            'edited by kmz for caj eksport dan gst
            Dim rstCaj1 As String = "Select sum(nilai_caj_Pelarasan_gst) as cekcaj FROM " + tablecaj + " WHERE Cara_Pembayaran='CEK' and kod_service_caj='Caj Perkhidmatan Ikan' and No_Barcode = '" & Trim(txtRujukan.Text) & "'"
            ...
            If SQLReader.HasRows Then
                ...
                Else
                    txtTotalCajKredit = Math.Round(CDec(SQLReader("cekcaj")), 2)
            ...
            'edited by kmz for caj eksport dan gst
            Dim pembayaran_gst As Decimal = 0.0
            Dim pembayaran_nilaicaj As Decimal = 0.0
            Dim baca As String = "Select (nilai_caj) as nilaicaj, (gst) as gst FROM " + tablecaj + " WHERE kod_service_caj='Caj Perkhidmatan Ikan' and No_Barcode = '" & Trim(txtRujukan.Text) & "'"
            SQLReader = opClass.DataReader(baca)
            If SQLReader.HasRows Then
                SQLReader.Read()

                pembayaran_nilaicaj = Trim(SQLReader("nilaicaj"))
                pembayaran_gst = Trim(SQLReader("gst"))
            End If
            SQLReader.Close()
            SQLReader = Nothing
            'end of edited by kmz for caj eksport
            ...
            'edited by kmz for caj eksport
            Dim rstPembayaranCaj As String = "Select no_barcode FROM " + tablecaj2 + " WHERE  No_Barcode = '" & Trim(txtRujukan.Text) & "'"
            SQLReader = opClass.DataReader(rstPembayaranCaj)


            If Not SQLReader.HasRows Then

                Dim rstPembayaranCaj1 As String = "Insert Into " + tablecaj2 + " (No_Barcode,No_Siri,No_Kenderaan," _
                & "Jumlah_Caj_Tunai,Jumlah_Caj_Kredit,No_Cek,Jumlah_Caj,Kod_Agen_Pengangkutan," _
                & "No_Pegawai,Nama_Pegawai,Status,site_code,pelarasangst, nilai_caj, gst) Values ('" & Trim(txtRujukan.Text) & "','" & Trim(NoSiriResit) & "','" & Trim(lblnokenderaan.Text) & "'," _
                & "'" & Trim(CDec(txtTotalCajTunai)) & "','" & Trim(CDec(txtTotalCajKredit)) & "','" & Trim(no_cek) & "','" & Trim(CDec(totalTransaksiSemasa)) & "','" & Trim(Kod_Agen.Value) & "'," _
                & "'" & Session("id_pegawai") & "','" & Session("namaPegawai") & "','JELAS','" & Right(Trim(txtRujukan.Text), 3) & "','" & CDec(arrBayarCol(4)) & "','" & Trim(CDec(pembayaran_nilaicaj)) & "','" & Trim(CDec(pembayaran_gst)) & "')"
