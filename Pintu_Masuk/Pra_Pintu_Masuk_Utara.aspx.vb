...
Partial Class Pra_Pintu_Masuk_Pintu_Masuk_Utara
        ...
        If Button2.Page.IsPostBack Then
            ...
            Else
                ...
                If barcode.Length > 0 Then
                    ...
                    sqlText = "INSERT INTO [W-ICCS].[dbo].[NO_SIRI_CAJ_MASUK]([barcode],pintu) VALUES('" & barcode & "','Import');select SCOPE_IDENTITY()"
                    ...
                    sqlText = "select count(*) as data from NO_SIRI_CAJ_MASUK where id<=" & noSiriCajResit & " and " & _
                            ...
                            "and datepart(yy,trans_date)=datepart(yy,getdate())) and pintu='Import'"
                    ...
                    Dim sqlText1 As String = "INSERT INTO [W-ICCS].[dbo].[SISTEM_Q]([No_Barcode]," & _
                                            ...
                                            "VALUES('" & barcode & "','IMPORT','" & noKenderaan.Text & "','','" & Session("namaPegawai") & "','Pintu Masuk Import','055')"
                    ...
                    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                    'sqlText = "select count(*) from pendaftaran_pintu where pintu_masuk ='Utara' " & _
                    '          "and(" & _
    ...
    Protected Sub PeriksaRujukan_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If PeriksaRujukan.Page.IsPostBack Then
            ...
            Dim sqlText As String = "SELECT TMP_PENDAFTARAN_PINTU.[Nombor_Barcode], TMP_PENDAFTARAN_PINTU.[No_Kenderaan], TMP_PENDAFTARAN_PINTU.[Kod_Agen_Pengangkutan], " & _
                                    ...
                                    "PENGISYTIHARAN_I.STATUS FROM [W-ICCS].[dbo].[TMP_PENDAFTARAN_PINTU] " & _
                                    "INNER JOIN AGEN_PENGANGKUTAN ON AGEN_PENGANGKUTAN.[Kod_Agen_Pengangkutan]=TMP_PENDAFTARAN_PINTU.[Kod_Agen_Pengangkutan] " & _
                                    "INNER JOIN PENGISYTIHARAN_I ON PENGISYTIHARAN_I.[No_Barcode]=TMP_PENDAFTARAN_PINTU.[Nombor_Barcode] " & _
                                    "WHERE TMP_PENDAFTARAN_PINTU.[Nombor_Barcode] = '" & NoRujukan.Text & "' and TMP_PENDAFTARAN_PINTU.[DeclarationStatus]='N'"
        ...
        If Button3.Page.IsPostBack Then
            ...
            Select Case data
                ...
                Case "1"
                    ...
                    Dim sqlText As String = "select kod_jenis_urusan,status from pengisyitiharan_all where no_barcode='" & TextBox1.Text & "'"
                    ...
                    ElseIf entStat.Trim = "A" Then
                        Select Case urusan
                            Case "003", "0017", "018"
                                strUrl = "../eksport/v_pengisyitiharan.aspx?&barcode=" & TextBox1.Text
                            Case "008"
                                strUrl = "../import/v_pengisyitiharan_pindahan.aspx?&barcode=" & TextBox1.Text
                            Case Else '"001", "002", "007", "013", "016", "004", "005", "006", "010", "011", "012", "014", "015"
                                strUrl = "../import/v_pengisyitiharan.aspx?&barcode=" & TextBox1.Text
                        End Select

                        Dim str = "input_box=confirm('Pengisytiharan Bagi Nombor Barcode Berikut " & TextBox1.Text & " Sudah Wujud!!! \nAdakah Anda Ingin Memaparkan Maklumat Pengisytiharan Tersebut? \nSila Tekan OK untuk YA CANCEL untuk TIDAK');"
                ...
                Case Else
                    ...
                    Response.Write("<script>location.href='" & opClass.VerifyBarcode(TextBox1.Text) & "';</script>")
