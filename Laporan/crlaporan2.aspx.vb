...
Partial Class _Default
    ...
    Private Sub ConfigureCrystalReports()
        ...
        If Request("type") = "H" Then
            ...
            ReportName = "Log_transaksi_smart_card_report.rpt"
            selectFormula = "{V_KAD2.Status_Kad}='A'  and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}= date(" & aDate & ")"
        ElseIf Request("type") = "B" Then
            ...
            ReportName = "Log_transaksi_smart_card_report(bulanan).rpt"
            selectFormula = "{V_KAD2.Status_Kad}='A'  and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
            'selectFormula = "LowerCase ({KAD_TRANSACTION_LOG.Transaction_Desc})='batal kad' or LowerCase ({KAD_TRANSACTION_LOG.Transaction_Desc})='Pembelian Kad Baru' and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
        ElseIf Request("type") = "T" Then
            aDate = startDate("01", Request("Tahun1"))
            bDate = EndDate("12", Request("Tahun2"))
            selectFormula = "{V_KAD2.Status_Kad}='A'  and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
            ReportName = "Log_transaksi_smart_card_report(tahunan).rpt"
        End If
        ...
        Dim reportPath As String = Server.MapPath("laporan kad prepaid report/rpt/" & ReportName)
