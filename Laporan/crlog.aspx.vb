...
Partial Class _Default
    ...
    Private Sub ConfigureCrystalReports()
        ...
        If Request("type") = "H" Then
            ...
            ReportName = "log.rpt"
            selectFormula = "{Gate_Activity_Log.entry_date}= date(" & aDate & ")"
        ElseIf Request("type") = "B" Then
            ...
            ReportName = "log(bulanan).rpt"
            selectFormula = "{Gate_Activity_Log.entry_date}>= date(" & aDate & ") and {Gate_Activity_Log.entry_date}<= date(" & bDate & ") "
        ElseIf Request("type") = "T" Then
            aDate = startDate("01", Request("Tahun1"))
            bDate = EndDate("12", Request("Tahun2"))
            selectFormula = "{Gate_Activity_Log.entry_date}>= date(" & aDate & ") and {Gate_Activity_Log.entry_date}<= date(" & bDate & ")"
            ReportName = "log(tahunan).rpt"
        End If
        ...
        Dim reportPath As String = Server.MapPath("rpt log/" & reportname)
