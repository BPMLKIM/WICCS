...
Partial Class Laporan_Papar_Laporan
    ...
    Protected Sub redisplay_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles redisplay.Click
        ...
        If Trim(sender.Id) = "redisplay" Then
            ...
            sqlText = "select pengisytiharan_i.status,no_invois.no_invois from pengisytiharan_i inner join no_invois on no_invois.no_barcode=pengisytiharan_i.no_barcode where pengisytiharan_i.no_barcode = '" & Trim(txtNoBarcode.Text) & "'"
            ...
            If ImEx > 0 Then
                ...
                If pilihan.SelectedValue = "SKPIE" Then

                    If ImEx = 1 Then
                        strUrl = "'eskpi.aspx?barcode=" & txtNoBarcode.Text & "&skpi=" & Trim(txtNoSkpi.Text) & "'"
                        'load skpi
                ...
                ElseIf pilihan.SelectedValue = "MasterSheet" Then
                    If ImEx = 1 Then
                        strUrl = "'crMasterSheetImport.aspx?barcode=" & txtNoBarcode.Text & "&invois_id=" & invois_id & "&SalinanMS=SALINAN-BUKIT KAYU HITAM'"
                        'load skpi
                        Response.Write("<script>window.open(" & strUrl & ");</script>")
                        response.Write("<script>self.location.href='SalinanSKPI_MsSheet.aspx'</script>")
                    ElseIf ImEx = 2 Then
