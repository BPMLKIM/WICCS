...
Partial Class _Default
    ...
    Private Sub ConfigureCrystalReports()
        ...
        If Request("type") = "H" Then
            ...
            ReportName = "Log_pembelian_smart_card_report.rpt"
            selectFormula = "lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'batal kad' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'reload prepaid' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'ganti kad baru' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'caj pintu masuk' and {V_KAD.status_kad}='A' and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
        ElseIf Request("type") = "B" Then
            ...
            ReportName = "Log_pembelian_smart_card_report(bulanan).rpt"
            selectFormula = "lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'batal kad' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'reload prepaid' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'ganti kad baru' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'caj pintu masuk' and {V_KAD.status_kad}='A' and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
        ElseIf Request("type") = "T" Then
            aDate = startDate("01", Request("Tahun1"))
            bDate = EndDate("12", Request("Tahun2"))
            selectFormula = "lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'batal kad' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'reload prepaid' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'ganti kad baru' and lowercase ({KAD_TRANSACTION_LOG.Transaction_Desc})<>'caj pintu masuk' and {V_KAD.status_kad}='A' and {KAD_TRANSACTION_LOG.Kadbatal_id}=0 and {kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
            ReportName = "Log_pembelian_smart_card_report(tahunan).rpt"
        End If
