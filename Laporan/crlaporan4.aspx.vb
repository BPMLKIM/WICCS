...
Partial Class _Default
    ...
    Private Sub ConfigureCrystalReports()
        ...
        If Request("type") = "H" Then
            ...
            selectFormula = "{kad_transaction_log.transaction_desc}<>'caj pintu masuk' and {kad_transaction_log.transaction_desc}<>'isu kad baru' and {kad_transaction_log.transaction_desc}<>'reload prepaid' and{kad_transaction_log.transaction_date}= date(" & aDate & ")"
            ReportName = "senarai_smart_card_report.rpt"
        ...
        ElseIf Request("type") = "B" Then
            ...
            ReportName = "senarai_smart_card_report(bulanan).rpt"
            selectFormula = "{kad_transaction_log.transaction_desc}<>'caj pintu masuk' and {kad_transaction_log.transaction_desc}<>'isu kad baru' and {kad_transaction_log.transaction_desc}<>'reload prepaid' and{kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
        ElseIf Request("type") = "T" Then
            aDate = startDate("01", Request("Tahun1"))
            bDate = EndDate("12", Request("Tahun2"))
            selectFormula = "{kad_transaction_log.transaction_desc}<>'caj pintu masuk' and {kad_transaction_log.transaction_desc}<>'isu kad baru' and {kad_transaction_log.transaction_desc}<>'reload prepaid' and{kad_transaction_log.transaction_date}>= date(" & aDate & ") and {kad_transaction_log.transaction_date}<= date(" & bDate & ")"
            ReportName = "senarai_smart_card_report(tahunan).rpt"
        End If
