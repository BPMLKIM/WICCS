...
Partial Class script1_tocTab
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'titles = "WICCS - Lembaga Kemajuan Ikan Malaysia"
        ...
        '---------------------------------tambahan untuk tentukan sistem bagi site yang mana-------------------------
        Dim tajuk As String = ""

        Dim sqlText1 As String = "SELECT [kod_ppi],[nama_ppi],[alamat_ppi],[title]" & _
                                "FROM [PUSATKOMPLEK_TITLE] "


        Dim sqlRdr As System.Data.SqlClient.SqlDataReader
        sqlRdr = opClass.DataReader(sqlText1)
        If sqlRdr.HasRows Then
            sqlRdr.Read()
            tajuk = sqlRdr.GetValue(3)
            sqlRdr.Close()
            sqlRdr = Nothing
        End If
        titles = "WICCS - " + tajuk
        ...
        sqlText = "SELECT DISTINCT menu_main.Menu_main, ISNULL(menu_main.link,'') as link, menu_main.menu_order, menu_main.module_id " & _
                  ...
                  "WHERE(group_setting.group_id = '" & Trim(Session("akses")) & "') order by menu_main.menu_order asc "
        jsReader = opClass.DataReader(sqlText)
