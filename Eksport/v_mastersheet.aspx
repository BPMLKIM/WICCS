<%@ Page Language="VB" AutoEventWireup="false" CodeFile="v_mastersheet.aspx.vb" Inherits="Eksport_v_mastersheet" %>
...
<%
    ...
    '          "LESEN ON PENGIMPORT.No_Lesen = LESEN.No_Lesen "
              ...
              sqlText = "SELECT RTRIM(PENGEKSPORT.Kod_PENGEKSPORT) as kodPENGEKSPORT, RTRIM(PENGEKSPORT.Kod_PENGEKSPORT) +'|'+ " & _
              "RTRIM(PENGEKSPORT.No_Lesen) + '|'+ CAST(DAY(LESEN.Tarikh_Tamat) AS nvarchar(200)) " & _
              "+ '/'+ case when MONTH(LESEN.Tarikh_Tamat) < 10 then '0'+cast(month(lesen.tarikh_tamat) as varchar(1)) else cast(month(lesen.tarikh_tamat) as varchar(2)) end   " & _
              "+ '/' + CAST(YEAR(LESEN.Tarikh_Tamat) as nvarchar(4)) AS Val," & _
              "RTRIM(PENGEKSPORT.Nama_Syarikat_eksport) as [Nama_Syarikat_Eksport] " & _
              "FROM PENGEKSPORT INNER JOIN " & _
              "LESEN ON PENGEKSPORT.No_Lesen = LESEN.No_Lesen "
    jsReader = opClass.DataReader(sqlText)
    Response.Write("<script>")
    Do While jsReader.Read
...
<script language="javascript" type="text/javascript">
 ...
 function processTable1()
 {
    ...
    alert(window.opener.document.all.arrayFishVal.value);
        ...
        */        
            ...
            {            
                ...
                if (arrIkanInfoDetails[16].replace(/\s+$/,"")!="" && parseFloat(arrIkanInfoDetails[16]) > 0)
                {
                    ...
                    totkdrberat[loc_index]= arrIkanInfoDetails[16];
                }    
                if (arrIkanInfoDetails[15].replace(/\s+$/,"")!="" && parseFloat(arrIkanInfoDetails[15]) > 0)
                {
                    totkdrekor[loc_index]= arrIkanInfoDetails[15];
                }
    ...
    <div>
        ...
        <table style="width: 100%">
            <tr>
                ...
                <td style="height: 15px; font-weight:bold;" >
                    URUSAN</td>
                <td style="height: 15px; font-weight:bold; width: 268px;" >
                    :
