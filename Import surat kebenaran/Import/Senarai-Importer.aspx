    ...
    <div>
        ...
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
                ...
                <asp:SqlDataSource ID="SenaraiImporter" runat="server" ConnectionString="<%$ ConnectionStrings:W-ICCS_Conn %>" SelectCommand="SELECT RTRIM(PENGIMPORT.Kod_Pengimport) AS kodPengimport, RTRIM(PENGIMPORT.Kod_Pengimport) + '|' + RTRIM(PENGIMPORT.No_Lesen) + '|' + CAST(DAY(LESEN.Tarikh_Tamat) AS nvarchar(200)) + '/' + CASE WHEN MONTH(LESEN.Tarikh_Tamat) < 10 THEN '0' + CAST(month(lesen.tarikh_tamat) AS varchar(1)) ELSE CAST(month(lesen.tarikh_tamat) AS varchar(2)) END + '/' + CAST(YEAR(LESEN.Tarikh_Tamat) AS nvarchar(4)) +'|'+  RTRIM(LESEN.No_Surat_Kebenaran) + '|' + CAST(DAY(LESEN.Tarikh_Tamat_surat) AS nvarchar(200)) + '/' + CASE WHEN MONTH(LESEN.Tarikh_Tamat_surat) < 10 THEN '0' + CAST(month(lesen.tarikh_tamat_surat) AS varchar(1)) ELSE CAST(month(lesen.tarikh_tamat_surat) AS varchar(2)) END + '/' + CAST(YEAR(LESEN.Tarikh_Tamat_surat) AS nvarchar(4)) AS Val, LEFT (RTRIM(PENGIMPORT.Nama_Syarikat_Import), 40) AS Nama_Syarikat_Import FROM PENGIMPORT INNER JOIN LESEN ON PENGIMPORT.No_Lesen = LESEN.No_Lesen WHERE (LEFT (RTRIM(PENGIMPORT.Nama_Syarikat_Import), 40) LIKE '' + @Nama_IMP + '%') ORDER BY Nama_Syarikat_Import">
                    <SelectParameters>
